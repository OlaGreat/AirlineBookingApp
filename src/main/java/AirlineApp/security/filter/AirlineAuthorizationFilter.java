...
public class AirlineAuthorizationFilter extends OncePerRequestFilter {
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if(getPublicPath().contains(request.getPathInfo())) filterChain.doFilter(request, response);
        else {
